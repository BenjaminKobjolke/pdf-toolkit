...
def test_default_when_missing(tmp_path: Path) -> None:
    ...
    assert state == UiState(menu_visible=False, toolbar_visible=False)


def test_save_then_load_round_trips(tmp_path: Path) -> None:
    store = _store(tmp_path)
    store.save(UiState(menu_visible=True, toolbar_visible=True))
    assert store.load() == UiState(menu_visible=True, toolbar_visible=True)


def test_fields_persist_independently(tmp_path: Path) -> None:
    store = _store(tmp_path)
    store.save(UiState(menu_visible=False, toolbar_visible=True))
    assert store.load() == UiState(menu_visible=False, toolbar_visible=True)


def test_corrupt_falls_back_to_default(tmp_path: Path) -> None:
    path = tmp_path / "ui_state.json"
    path.write_text("{ not json", encoding="utf-8")
    assert UiStateStore(path).load() == UiState(menu_visible=False)
