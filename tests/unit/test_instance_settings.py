...
def test_default_is_focus_on_forward() -> None:
    assert InstanceSettings().focus_on_forward is True
...
def test_round_trip_focus_on_forward(tmp_path: Path) -> None:
    store = InstanceSettingsStore(gui_backend(tmp_path))
    store.save(InstanceSettings(focus_on_forward=False))
    assert store.load() == InstanceSettings(focus_on_forward=False)
