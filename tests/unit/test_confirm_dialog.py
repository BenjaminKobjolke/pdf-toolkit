...
from app.gui.confirm_dialog import ConfirmDialog, ConfirmSpec, DialogResult, Severity, show_message
...
def test_primary_click_returns_primary(qapp: object) -> None:
    dialog = ConfirmDialog(ConfirmSpec(title="T", message="M", primary="Yes", secondary="No"))
    _click(dialog, "Yes")
...
def test_cancel_click_returns_cancel(qapp: object) -> None:
    dialog = ConfirmDialog(
        ConfirmSpec(title="T", message="M", primary="Save", secondary="Discard", cancel="Cancel")
    )
...
def test_show_message_prepends_severity_prefix(
    ...
    captured: list[ConfirmSpec] = []

    class FakeDialog:
        def __init__(self, spec: ConfirmSpec, parent: object = None) -> None:
    ...
    assert captured[0].message.startswith("✕ ")
    assert captured[0].message.endswith("boom")
