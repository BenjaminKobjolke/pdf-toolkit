...
def field_commands(window: MainWindow) -> list[Command]:
    ...
    c = commands
    pdf = commands.PDF_ONLY  # overlay editing exists only on PDFs
    return [
        Command(
            c.FIELD_CHANGE_TEXT, strings.CMD_FIELD_CHANGE_TEXT, fields.change_text, has_field, pdf
        ),
        Command(c.FIELD_FONT_SIZE, strings.CMD_FIELD_FONT_SIZE, fields.change_size, has_field, pdf),
        Command(
            c.FIELD_FONT_FAMILY, strings.CMD_FIELD_FONT_FAMILY, fields.change_font, has_field, pdf
        ),
        Command(
            c.FIELD_TEXT_COLOR,
            strings.CMD_FIELD_TEXT_COLOR,
            fields.change_text_color,
            has_field,
            pdf,
        ),
        Command(
            c.FIELD_BG_COLOR, strings.CMD_FIELD_BG_COLOR, fields.change_bg_color, has_field, pdf
        ),
        Command(
            c.FIELD_TOGGLE_BOLD, strings.CMD_FIELD_TOGGLE_BOLD, fields.toggle_bold, has_field, pdf
        ),
        Command(
            c.FIELD_TOGGLE_ITALIC,
            strings.CMD_FIELD_TOGGLE_ITALIC,
            fields.toggle_italic,
            has_field,
            pdf,
        ),
        Command(c.FIELD_DELETE, strings.CMD_FIELD_DELETE, fields.delete, has_field, pdf),
    ]
...
def image_commands(window: MainWindow) -> list[Command]:
    ...
    pdf = commands.PDF_ONLY
    return [
        Command(commands.IMAGE_SCALE, strings.CMD_IMAGE_SCALE, images.change_scale, has_image, pdf),
        Command(commands.IMAGE_DELETE, strings.CMD_IMAGE_DELETE, images.delete, has_image, pdf),
    ]
...
def rectangle_commands(window: MainWindow, has_doc: Predicate) -> list[Command]:
    ...
    pdf = commands.PDF_ONLY
    return [
        Command(ADD_RECT, s.CMD_ADD_RECT, window.add_rect, has_doc, pdf),
        Command(RECT_FILL_COLOR, s.CMD_RECT_FILL_COLOR, rects.change_fill_color, has_rect, pdf),
        Command(RECT_WIDTH, s.CMD_RECT_WIDTH, rects.change_width, has_rect, pdf),
        Command(RECT_HEIGHT, s.CMD_RECT_HEIGHT, rects.change_height, has_rect, pdf),
        Command(RECT_DELETE, s.CMD_RECT_DELETE, rects.delete, has_rect, pdf),
    ]
...
def layer_commands(window: MainWindow) -> list[Command]:
    ...
    pdf = commands.PDF_ONLY
    return [
        Command(LAYER_FORWARD, s.CMD_LAYER_FORWARD, layers.move_forward, has_layer, pdf),
        Command(LAYER_BACKWARD, s.CMD_LAYER_BACKWARD, layers.move_backward, has_layer, pdf),
        Command(LAYER_TO_FRONT, s.CMD_LAYER_TO_FRONT, layers.bring_to_front, has_layer, pdf),
        Command(LAYER_TO_BACK, s.CMD_LAYER_TO_BACK, layers.send_to_back, has_layer, pdf),
    ]
